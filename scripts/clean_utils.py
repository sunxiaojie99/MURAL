...
def remove_control_char(data):
    ...
    chars_dict = {c: "" for c in global_chars_list if unicodedata.category(c)[
        0] == "C"}
    data = list(
        map(
            lambda x: " ".join([_make_cleaning(i, chars_dict)
                               for i in x.split()]), data
        )
    )
    return data
...
def strip_accents(data):
    """Nonspacing_Mark"""
    if type(data) == list:
        return [strip_accents(d) for d in data]
    elif type(data) == str:
...
def remove_duplicate(data):
    ...
    data = list(
        map(
            lambda x: " ".join([_make_dict_cleaning(i, temp_dict)
                               for i in x.split()]),
            data,
        )
    )
...
def seperate_spam_chars(data):
    ...
    for word in temp_vocab:
        if (
            (len(re.compile("[a-zA-Z0-9\-\.\,\/']").sub("",
             word)) / len(word) > 0.6)
            and (len(Counter(word)) == 1)
            and (len(word) > 2)
        ):
            temp_dict[word] = " ".join(
                [" " + next(iter(Counter(word).keys())) +
                 " " for i in range(3)]
            )
    print(temp_dict)
    data = list(
...
def seperate_start_word_punctuations(data):
    ...
    temp_vocab = [k for k in temp_vocab if (
        _check_replace(k)) and (not k[0].isalnum())]
    temp_dict = {}
    for word in temp_vocab:
        new_word = word
        for i in range(len(word)):
...
def clean_open_holded_words(data):
    ...
    data = list(map(lambda x: " ".join(
        [temp_dict.get(i, i) for i in x.split()]), data))
